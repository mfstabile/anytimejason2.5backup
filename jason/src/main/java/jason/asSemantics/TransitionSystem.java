...
import jason.bb.PriorizedPerceptionList;
import jason.bb.StructureWrapperForLiteral;
import jason.infra.centralised.CentralisedAgArchAnytimeAsynchronous;
import jason.infra.centralised.EmpiricalDistributionsController;
import jason.infra.centralised.EmpiricalDistributionsProfiler;
...
public class TransitionSystem {
    ...
    private State         stepDeliberate  = State.ProcAct;
    ...
    private int           tfboce = 0; // number of taskForBeginOfCycle executed in sequentially
    ...
    private EmpiricalDistributionsController edc;
    private EmpiricalDistributionsProfiler edp;
    private Object  syncPercept            = new Object();
    private Object  syncDelib              = new Object();
    private Object  syncAct                = new Object();
...
//        Logger LOG = Logger.getLogger(agArch.getAgName());
//        LOG.info("selecting intention");
...
//it = getC().getAllIntentions();
//getLogger().info("Intentions:");
//while (it.hasNext()){
//    getLogger().info(it.next().toString());
//}
//getLogger().info("End of Intentions");
            return;
        }
...
//  Logger LOG = Logger.getLogger(agArch.getAgName());
//  LOG.info("Executing action " + h + "<->" + body );
...
//                LOG.info("WRONG_-_-_Executing action " + h + "<->" + body );
    ...
    public void anytimeDeliberate() {
        Logger LOG = Logger.getLogger(agArch.getAgName());
        while (getUserAgArch().isRunning()) {
            if (edc.hasTime()) {
                //LOG.info("Executing deliberate");
                try {

                    if (stepDeliberate == State.ProcAct) {
//                      if (!edc.hasTime("taskForBeginOfCycle")) {
//                          break;
//                      }
                        if (tfboce == 0)
                            C.resetDeliberate();
                        ...
                        if (r != null) {
                            r.run(); // It is processed only things related to operations on goals/intentions
                                        // resumed/suspended/finished It can be placed in the deliberate stage, but the
                                        // problem is the sleep when the synchronous execution is adopted
                            r = taskForBeginOfCycle.poll();
                            tfboce++;
                            continue;
                        } else {
...
//                      if (!edc.hasTime(stepDeliberate.toString())) {
//                          break;
//                      }
                        applySemanticRuleDeliberate();
                    }
                    ...
                    logger.log(Level.SEVERE,
                            "*** ERROR in the transition system (deliberate). " + C + "\nCreating a new C!", e);
                    C.create();
                }

            } else {
                try {
//                    LOG.info("waiting deliberate");
//                    syncDelib.wait(50);
                    Thread.sleep(50);
//                    LOG.info("stopped waiting deliberate");
...
//        LOG.severe("outside of anytimeDeliberate");
    }
            ...
            while (getUserAgArch().isRunning()) {
                ...
                if(stepDeliberate == State.ProcAct) {
                    ...
                    if (r != null) {
                        ...
                        edp.addTime("taskForBeginOfCycle", time);
                        continue;
                    }else {
                        stepDeliberate = State.SelEv;
                        tfboce=0;
                    ...
                    applySemanticRuleDeliberate();
                    long time = System.nanoTime() - begin;
                    edp.addTime(stepDeliberate.toString(), time);
                    if (stepDeliberate == State.ProcAct) {
                        tfboce=0;
                        break;
                    }
...
//        Logger LOG = Logger.getLogger(agArch.getAgName());
        while (getUserAgArch().isRunning()) {
            ...
            if (edc.hasTime()) {
                //LOG.info("executing anytimeAct");
                try {

//                    if (!edc.hasTime(stepAct.toString())) {
//                        break;
//                    }
...
//        LOG.severe("outside of anytimeAct");
    }
    ...
    public void anytimeActProfiler() {
        try {
            while (getUserAgArch().isRunning()) {
                ...
                long time = System.nanoTime() - begin;
                edp.addTime(stepAct.toString(), time);
...
//          ActionExec action = C.getAPQ().poll();
//          if (action != null) {
//              C.addPendingAction(action);
//              // We need to send a wrapper for FA to the user so that add method then calls C.addFA (which control atomic things)
//              getUserAgArch().act(action); //, C.getFeedbackActionsWrapper());
//          }
    ...
    */
    ...
    public void anytimeBUF(Collection<Literal> percepts, DefaultBeliefBase bb, String beliefPriority) {
//        Logger LOG = Logger.getLogger(agArch.getAgName());
        
        if (percepts == null) {
//            LOG.info("null percepts");
            return;
        }

//        PriorizedPerceptionList orderedPercepts = new PriorizedPerceptionList(percepts.iterator(), beliefPriority);

        DefaultBeliefBase oldBB = (DefaultBeliefBase) bb.clone();

        bb.clear();

        Iterator<Literal> perceptsIter = percepts.iterator();
//        Iterator<Literal> perceptsIter = orderedPercepts.iterator();

        while (perceptsIter.hasNext()) {
//            if(!edc.hasTime("AddBelief"))return;
            Literal l = perceptsIter.next();
            l.addAnnot(BeliefBase.TPercept);
            bb.add(l);
            if (oldBB.contains(l) == null) {
//                LOG.info("added event "+l);
                this.updateEvents(new Event(new Trigger(TEOperator.add, TEType.belief, l), Intention.EmptyInt));
            } else {
                oldBB.remove(l);
            }
        }

        Iterator<Literal> perceptsInBB = oldBB.iterator();
        Set<Literal> oldPercepts = oldBB.getPerceptsSet();
        
        while (perceptsInBB.hasNext()) {
//            if(!edc.hasTime("RemoveBelief"))return;
            Literal l = perceptsInBB.next();
            if (oldPercepts.contains(l)) { // l is not perceived anymore
                // new version (it is sure that l is in BB, only clone l when the event is
                // relevant)
//                LOG.info("removed event "+l);
                Trigger te = new Trigger(TEOperator.del, TEType.belief, l);
                if (this.getC().hasListener() || ag.getPL().hasCandidatePlan(te)) {
                    l = ASSyntax.createLiteral(l.getFunctor(), l.getTermsArray());
                    l.addAnnot(BeliefBase.TPercept);
                    te.setLiteral(l);
                    this.getC().addEvent(new Event(te, Intention.EmptyInt));
                }
            }
            else {
                bb.add(l);
    ...
    public void anytimeBUFProfiler(Collection<Literal> percepts, DefaultBeliefBase bb, String beliefPriority) {
        if (percepts == null) {
            return;
        }
        ...
        while (perceptsInBB.hasNext()) {
            long begin = System.nanoTime();
            Literal l = perceptsInBB.next();
//            if (l.subjectToBUF() && oldPercepts.contains(l)) {
            if (oldPercepts.contains(l)) {
                // l is not perceived anymore
                // new version (it is sure that l is in BB, only clone l when the event is
                // relevant)
                Trigger te = new Trigger(TEOperator.del, TEType.belief, l);
                if (this.getC().hasListener() || ag.getPL().hasCandidatePlan(te)) {
            ...
            long time = System.nanoTime() - begin;
            edp.addTime("RemoveBelief", time);
        }
    }

    public void anytimeSense() {
        Logger LOG = Logger.getLogger(agArch.getAgName());
        LOG.info("anytimeSense starting");
        while (getUserAgArch().isRunning()) {
//            if(edc.hasTime()) {
            if(true) {
//                LOG.info("running anytimeSense");
                try {
                    if (logger.isLoggable(Level.FINE)) logger.fine("Start anytime sensing");
                    C.resetSense();
                    ...
                    if (nrcslbr >= setts.nrcbp()) {
                        ...
                        synchronized (C.syncApPlanSense) {
//                            ag.buf();
                            Collection<Literal> percepts = getUserAgArch().perceive();
                            BeliefBase bb = ag.getBB();
                            CentralisedAgArchAnytimeAsynchronous agArch2 = (CentralisedAgArchAnytimeAsynchronous) agArch;
                            
                            String beliefPriority;
                            if(edc.hasTime("getBeliefPriority")){
                                beliefPriority = agArch2.getBeliefPriority();
                                
                                if (bb instanceof DefaultBeliefBase) {
//                                    anytimeBUF(percepts, (DefaultBeliefBase) bb, beliefPriority);
                                    ag.buf(getUserAgArch().perceive());
                                }else {
                                    logger.log(Level.SEVERE, "*** Anytime Architecture does not support custom Belief Bases. Behaviour is uncertain.");
                                    ag.buf(getUserAgArch().perceive());
                                }
                            }
                        }
                        if(edc.hasTime("checkMail")){
                            getUserAgArch().checkMail();    
                        }else {
                            continue;
                        }
                    ...
                    stepSense = State.StartRC;
                    while (stepSense != State.SelEv && getUserAgArch().isRunning() && edc.hasTime(State.StartRC.toString())) {
                        applySemanticRuleSense();
                    }
...
//        LOG.severe("outside of anytimeSense");
    }
...
//                    ag.buf();
                    ...
                    beliefPriority = agArch2.getBeliefPriority();
                    long time = System.nanoTime() - begin;
                    edp.addTime("getBeliefPriority", time);
                    if (bb instanceof DefaultBeliefBase) {
                        anytimeBUFProfiler(percepts, (DefaultBeliefBase) bb, beliefPriority);
                    }else {
                        logger.log(Level.SEVERE, "*** Anytime Architecture does not support custom Belief Bases. Behaviour is uncertain.");
                        ag.buf(getUserAgArch().perceive());
                    }
            ...
            while (stepSense != State.SelEv && getUserAgArch().isRunning()) {
                ...
                applySemanticRuleSense();
                long time = System.nanoTime() - begin;
                edp.addTime(State.StartRC.toString(), time);
            }
    ...
    public void setEDC(EmpiricalDistributionsController edc) {
        this.edc = edc;
    }
    ...
    public void setEDP(EmpiricalDistributionsProfiler edp) {
        this.edp = edp;
    }
}
